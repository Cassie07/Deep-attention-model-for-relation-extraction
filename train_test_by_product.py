...
# return is a list of mentions_id: '9151bec746be1369933e0711724fabba.(41, 24).Apex'
def load_pair_name(dir,pair_id_dir,test_product):
        ...
        if i=='test_dataset':
            ...
            for j in label:
                ...
                for k in product:
                    test_dataset_pair=[]
                    train_dataset_pair=[]
                    if k==test_product:  # add in test data
                        ...
                        #print(test_dataset_pair)
                        for m in test_dataset_pair:  # match pair and find all mention_ids for pairs
                            dic_product=pair_mid[k.split('.')[0]] # key=product
                            dic_label=dic_product[j+'.json'] # key= label
                            for n in dic_label[m]:
                                test_dataset.append(n) # list of mention ids
                    else:  # add in train data
                        text_file = open(path2+'/'+k, "r")
                        lines = text_file.readlines()  # list of pairs: ['pair1/n','pair2/n',...]
                        for l in lines:  # omit '/n'
...
#train_d,test_d=load_pair_name(FLAGS.dir,FLAGS.pair_id_dir,'Canon G3.txt')
